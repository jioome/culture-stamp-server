package com.culturestamp.back.auth.config.util;
...
public class JWTUtils {
    ...
    public String createToken(UserServiceResponse userServiceResponse, boolean rememberMe) {
        long now = (new Date()).getTime();
        Date validity = rememberMe ? new Date(now + TOKEN_VALIDITY_REMEMBER) : new Date(now + TOKEN_VALIDITY);
        Map<String, Object> claims = new HashMap<>();
        claims.put("role", userServiceResponse.getRole());

        return Jwts.builder()
                .setSubject(userServiceResponse.getUserId().toString())
